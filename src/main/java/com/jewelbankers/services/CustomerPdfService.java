...
/*
 * @Service public class CustomerPdfService {
 * 
 * @Autowired SettingsUtillity settingsUtillity;
 * 
 ...
 * private static final String OUTPUT_PATH = "bills";
 * 
 * public ByteArrayInputStream generateCustomerBillPdf(Bill bill, Map<String,
 * String> settingsMap) throws IOException, DocumentException { // Ensure the
 * output directory exists File dir = new File(OUTPUT_PATH);
 * ByteArrayOutputStream baos = new ByteArrayOutputStream();
 * 
 * if (!dir.exists()) { dir.mkdirs(); }
 * 
 * // Set output file path String outputFileName = "bill_" +
 * bill.getBillSerial() + "_" + bill.getBillNo() + ".pdf"; String outputFilePath
 * = OUTPUT_PATH + File.separator + outputFileName;
 * 
 * PdfReader reader = null; PdfStamper stamper = null; FileOutputStream fos =
 * null;
 * 
 * try { // Ensure essential settings are not null String shopName =
 * settingsMap.getOrDefault("SHOP_NAME", "Shop Name"); String shopLine1 =
 * settingsMap.getOrDefault("SHOP_NO", "") + " " +
 * settingsMap.getOrDefault("SHOP_STREET", ""); String shopLine2 =
 * settingsMap.getOrDefault("SHOP_AREA", "") + " " +
 * settingsMap.getOrDefault("SHOP_CITY", ""); String shopLine3 =
 * settingsMap.getOrDefault("SHOP_STATE", "") + " - " +
 * settingsMap.getOrDefault("SHOP_PINCODE", "");
 * 
 * // Read the template PDF reader = new PdfReader(TEMPLATE_PATH);
 * 
 * // Create a new PDF with the content filled fos = new
 * FileOutputStream(outputFilePath); stamper = new PdfStamper(reader, fos);
 * 
 * // Set the page size to A5 Rectangle a5Size = PageSize.A5;
 * stamper.getWriter().setPageSize(a5Size); // Force the page size to A5
 * 
 * // Get the PDF content PdfContentByte content = stamper.getOverContent(1);
 * 
 * // Set the font and size BaseFont baseFont =
 * BaseFont.createFont(BaseFont.HELVETICA, BaseFont.WINANSI, BaseFont.EMBEDDED);
 * Font boldFont = new Font(baseFont, 14, Font.BOLD); Font regularFont = new
 * Font(baseFont, 12, Font.NORMAL);
 * 
 * // Shop Name (BOLD and CENTERED) content.beginText();
 * content.setFontAndSize(boldFont.getBaseFont(), 16); // Bold font for Shop
 * Name, you can adjust the font size as needed
 * content.setColorFill(BaseColor.BLACK);
 * content.setTextRenderingMode(PdfContentByte.TEXT_RENDER_MODE_FILL_STROKE); //
 * Bold rendering mode content.setLineWidth(0.5f);
 * content.showTextAligned(Element.ALIGN_CENTER, shopName, 300, 745, 0); //
 * Center alignment with coordinates (300, 745) content.endText();
 * 
 * // Shop Address (Regular Font, split into three lines) content.beginText();
 * content.setFontAndSize(regularFont.getBaseFont(), 12);
 * content.showTextAligned(Element.ALIGN_CENTER, shopLine1, 300, 730, 0); //
 * First line content.showTextAligned(Element.ALIGN_CENTER, shopLine2, 300, 715,
 * 0); // Second line content.showTextAligned(Element.ALIGN_CENTER, shopLine3,
 * 300, 700, 0); // Third line content.endText();
 * 
 * // Bill Serial and No (Ensure they are not null) content.beginText(); // Set
 * font to bold, size to 14, and make text black
 ...
 * + (bill.getBillNo() != null ? bill.getBillNo().toString() : "N/A"), 85, 665,
 * 0); content.endText();
 * 
 * 
 * // // Define the desired output format DateTimeFormatter outputFormatter =
 * DateTimeFormatter.ofPattern("dd/MM/yyyy"); // Desired output format
 * 
 * // Assuming bill.getBillDate() returns a LocalDate LocalDate billDate =
 * bill.getBillDate(); // This should return the date as a LocalDate String
 * billDateFormatted;
 * 
 * // Format the LocalDate object into the desired format if (billDate != null)
 * { billDateFormatted = billDate.format(outputFormatter); } else {
 * billDateFormatted = "N/A"; // Default value if the bill date is null }
 * 
 * // Bill Date (Ensure it's not null) content.beginText();
 * content.setFontAndSize(regularFont.getBaseFont(), 13);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT, billDateFormatted, 455,
 * 665, 0); content.endText();
 * 
 * // Check if the customer has a photo and retrieve it as byte array byte[]
 * customerPhoto = bill.getCustomer() != null ? bill.getCustomer().getPhoto() :
 * null;
 * 
 * if (customerPhoto != null) { try { // Convert the byte array to an iText
 * Image Image photo = Image.getInstance(customerPhoto);
 * 
 ...
 * 
 * // Add the photo to the PDF content content.addImage(photo); } catch
 * (Exception e) { System.out.println("Failed to add photo: " + e.getMessage());
 * } }
 * 
 * 
 * // (Ensure it's not null) String customerName = bill.getCustomer() != null &&
 * bill.getCustomer().getCustomerName() != null ?
 * bill.getCustomer().getCustomerName() : "Customer Name"; content.beginText();
 * content.setFontAndSize(boldFont.getBaseFont(), 14);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT, customerName, 35, 610, 0);
 * content.endText();
 * 
 * // Customer Address (Split into lines and ensure it's not null) String[]
 * addressLines = bill.getCustomer() != null && bill.getCustomer().getAddress()
 * != null ? bill.getCustomer().getAddress().split(",") : new
 * String[]{"Address Line 1", "Address Line 2", "Address Line 3"};
 * 
 * content.beginText(); content.setFontAndSize(regularFont.getBaseFont(), 12);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT, addressLines.length > 0 ?
 * addressLines[0] : "", 35, 590, 0); // First line of address
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT, addressLines.length > 1 ?
 * addressLines[1] : "", 30, 570, 0); // Second line of address
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT, addressLines.length > 2 ?
 * addressLines[2] : "", 30, 550, 0); // Third line of address
 * content.endText();
 * 
 * // Bill Details (Ensure non-null and non-empty lists) if
 * (bill.getBillDetails() != null && !bill.getBillDetails().isEmpty()) {
 * content.beginText(); content.setFontAndSize(regularFont.getBaseFont(), 14);
 ...
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT,
 * String.valueOf(bill.getBillDetails().get(0).getProductQuantity()), 390, 470,
 * 0); content.endText(); }
 * 
 * // Grams (Ensure non-null) content.beginText();
 * content.setFontAndSize(regularFont.getBaseFont(), 14);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT,
 * String.valueOf(bill.getGrams() != null ? bill.getGrams() : "N/A"), 495, 479,
 * 0); content.endText();
 * 
 * // NetWt->Grams (Ensure non-null) content.beginText();
 * content.setFontAndSize(regularFont.getBaseFont(), 14);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT,
 * String.valueOf(bill.getGrams() != null ? bill.getGrams() : "N/A"), 495, 387,
 * 0); content.endText();
 * 
 * // Amount Value (Ensure non-null) content.beginText();
 ...
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT, "Rs. " +
 * String.valueOf(bill.getAmount() != null ? bill.getAmount() : "N/A"), 280,
 * 352, 0); content.endText();
 * 
 * // AmountInWords Value (Ensure non-null) content.beginText();
 * content.setFontAndSize(regularFont.getBaseFont(), 14);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT,
 * String.valueOf(bill.getAmountInWords() != null ? bill.getAmountInWords() :
 * "N/A"), 30, 330, 0); content.endText();
 * 
 * // Present Value (Ensure non-null) content.beginText();
 * content.setFontAndSize(regularFont.getBaseFont(), 14);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT,
 * String.valueOf(bill.getPresentValue() != null ? bill.getPresentValue() :
 * "N/A"), 495, 335, 0); content.endText();
 * 
 * // Fetch settings from the database List<Settings> settingsList =
 * settingsService.getSettings();
 * 
 * // Convert the settings list to a map settingsMap =
 * settingsUtillity.convertListToMap(settingsList);
 * 
 * // Fetch pledge rules using the utility method String pledgeRules =
 * settingsUtillity.getPledgeRules(settingsMap);
 * 
 * // Check if pledgeRules is null and set a default if necessary if
 * (pledgeRules == null) { pledgeRules =
 * "The final due date for pledged items is only 1 year and 7 days: Interest must be paid once every three months without fail. The last date to redeem the pledged items: Reason for borrowing: My monthly income:"
 * ; // Default text }
 * 
 * // Manually add line breaks at appropriate places in the string String[]
 * lines = { "The final due date for pledged items is only 1 year and 7 days:",
 * "Interest must be paid once every three months without fail.",
 * "The last date to redeem the pledged items:", "Reason for borrowing:",
 * "My monthly income:" };
 * 
 * // Set the starting position for the first line float yPosition = 300; //
 * Starting Y position for A5 float lineSpacing = 20; // Space between lines
 * (adjust as necessary)
 * 
 * content.beginText(); content.setFontAndSize(boldFont.getBaseFont(), 14);
 * content.setColorFill(new BaseColor(0, 0, 0));
 * 
 * // Display each line in the PDF for (String line : lines) { if (yPosition <
 * 40) { // Check if the Y position is below a threshold break; // Stop if we're
 * running out of space } content.showTextAligned(Element.ALIGN_LEFT, line, 25,
 * yPosition, 0); yPosition -= lineSpacing; // Move down for the next line }
 * 
 * content.endText();
 * 
 * 
 * content.beginText(); content.setFontAndSize(boldFont.getBaseFont(), 12);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT, bill.getBillSerial() + " "
 * + (bill.getBillNo() != null ? bill.getBillNo().toString() : "N/A"), 147, 88,
 * 0); content.endText();
 * 
 * // Customer Name (Ensure it's not null) // String customerName =
 * bill.getCustomer() != null && bill.getCustomer().getCustomerName() != null
 * //? bill.getCustomer().getCustomerName() : "Customer Name";
 * content.beginText(); content.setFontAndSize(boldFont.getBaseFont(), 12);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT, customerName, 140, 70, 0);
 * content.endText();
 * 
 * // Define the desired output format DateTimeFormatter outputFormatter1 =
 * DateTimeFormatter.ofPattern("dd/MM/yyyy"); // Desired output format
 * 
 * // Assuming bill.getBillDate() returns a LocalDate LocalDate billDate2 =
 * bill.getBillDate(); // This should return the date as a LocalDate String
 * billDateFormatted1;
 * 
 * // Format the LocalDate object into the desired format if (billDate != null)
 * { billDateFormatted1 = billDate2.format(outputFormatter1); } else {
 * billDateFormatted1 = "N/A"; // Default value if the bill date is null }
 * 
 * // Displaying the bill date content.beginText();
 * content.setFontAndSize(regularFont.getBaseFont(), 12);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT, billDateFormatted1, 140,
 * 55, 0); content.endText();
 * 
 * content.beginText(); content.setFontAndSize(regularFont.getBaseFont(), 12);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT,
 * String.valueOf(bill.getGrams() != null ? bill.getGrams() : "N/A"), 245, 55,
 * 0); content.endText();
 * 
 * content.beginText(); content.setFontAndSize(boldFont.getBaseFont(), 12);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT, "Rs. " +
 * String.valueOf(bill.getAmount() != null ? bill.getAmount() : "N/A"), 155, 39,
 * 0); content.endText();
 * 
 * content.beginText(); content.setFontAndSize(regularFont.getBaseFont(), 12);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT,
 ...
 * content.endText();
 * 
 * content.beginText(); content.setFontAndSize(regularFont.getBaseFont(), 13);
 * content.showTextAligned(PdfContentByte.ALIGN_LEFT,
 * String.valueOf(bill.getMonthlyIncome() != null ? bill.getMonthlyIncome() :
 * "N/A"), 175, 220, 0); content.endText();
 * 
 * LocalDate billDate1 = bill.getBillDate(); // Assuming billDate is a LocalDate
 * LocalDate dueDate = billDate1.plusYears(1).plusDays(7); // Add 1 year and 7
 * days
 * 
 * // Format the due date for displaying in the PDF DateTimeFormatter formatter
 * = DateTimeFormatter.ofPattern("dd-MM-yyyy", Locale.ENGLISH); String
 * dueDateFormatted = dueDate.format(formatter);
 * 
 * // Set text color to red for dueDateFormatted content.beginText();
 * content.setFontAndSize(regularFont.getBaseFont(), 12); // Set font for
 * regular text content.setColorFill(BaseColor.CYAN); // Set text color to red
 * content.showTextAligned(Element.ALIGN_LEFT, dueDateFormatted, 425, 300, 0);
 * // Adjust the coordinates as needed content.endText();
 * 
 * // Close the PDF Stamper and flush the data stamper.close();
 * 
 * // Convert generated file to ByteArrayInputStream for return File file = new
 * File(outputFilePath); byte[] fileBytes =
 * java.nio.file.Files.readAllBytes(file.toPath()); return new
 * ByteArrayInputStream(fileBytes);
 * 
 * } catch (IOException | DocumentException e) { e.printStackTrace(); throw e; }
 * finally { if (fos != null) { fos.close(); } if (reader != null) {
 * reader.close(); } } } }
 */
